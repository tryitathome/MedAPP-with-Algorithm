...
export interface SegmentationResult {
  ...
  overlayImageObjectPath?: string;
}

export class SegmentationService {
  // 工作区根目录（从编译后 dist/* 下回溯）
  private readonly workspaceRoot = path.resolve(__dirname, '../../../../..');
  // 允许通过环境变量覆盖，保留旧的硬编码作为兜底，避免现有环境立即失效
  // Python 解释器：优先使用 .env 中的 MMDET_PYTHON_EXE_PATH；否则退回到系统可执行的 "python"
  private readonly pythonPath = process.env.MMDET_PYTHON_EXE_PATH || 'python';
  private readonly mmdetectionDir = process.env.MMDET_DIR || path.join(this.workspaceRoot, 'MMDETECTION_mini');
  private readonly scriptPath = process.env.MMDET_SCRIPT_PATH || path.join(this.mmdetectionDir, 'image_demo.py');
  ...
  // RTX 50-series GPUs require a newer PyTorch/MMCV build. Allow CPU fallback
  // without changing the inference script, whose default device is cuda:0.
  private readonly device = process.env.MMDET_DEVICE || 'cuda:0';
  private readonly timeoutMs = Number(process.env.MMDET_TIMEOUT_MS || 600000);
  ...
  async runSegmentation(request: SegmentationRequest): Promise<SegmentationResult> {
    ...
    try {
      ...
      const overlayImageObjectPath = await objectStorageService.uploadLocalFile(
        finalImagePath,
        objectStorageService.resultObjectPath(finalFileName)
      );
